...
`)?Qn(t,e):t.includes(`
`)?`
`+Qn(t,e):(n.endsWith(" ")?"":" ")+t;Kt.indentComment=Qn;Kt.lineComment=ud;Kt.stringifyComment=fd});var Is=_(it=>{"use strict";var pd="flow",Zn="block",jt="quoted";function md(n,e,t="flow",{indentAtStart:i,lineWidth:r=80,minContentWidth:s=20,onFold:o,onOverflow:a}={}){if(!r||r<0)return n;r<s&&(s=0);let c=Math.max(1+s,1+r-e.length);if(n.length<=c)return n;let l=[],u={},d=r-e.length;typeof i=="number"&&(i>r-Math.max(2,s)?l.push(0):d=r-i);let f,m,g=!1,p=-1,y=-1,T=-1;t===Zn&&(p=Os(n,p,e.length),p!==-1&&(d=p+c));for(let S;S=n[p+=1];){if(t===jt&&S==="\\"){switch(y=p,n[p+1]){case"x":p+=3;break;case"u":p+=5;break;case"U":p+=9;break;default:p+=1}T=p}if(S===`
`)t===Zn&&(p=Os(n,p,e.length)),d=p+e.length+c,f=void 0;else{if(S===" "&&m&&m!==" "&&m!==`
`&&m!=="	"){let w=n[p+1];w&&w!==" "&&w!==`
`&&w!=="	"&&(f=p)}if(p>=d)if(f)l.push(f),d=f+c,f=void 0;else if(t===jt){for(;m===" "||m==="	";)m=S,S=n[p+=1],g=!0;let w=p>T+1?p-2:y-1;if(u[w])return n;l.push(w),u[w]=!0,d=w+c,f=void 0}else g=!0}m=S}if(g&&a&&a(),l.length===0)return n;o&&o();let E=n.slice(0,l[0]);for(let S=0;S<l.length;++S){let w=l[S],k=l[S+1]||n.length;w===0?E=`
...
`);i=e,r=e+1,s=n[r]}return i}it.FOLD_BLOCK=Zn;it.FOLD_FLOW=pd;it.FOLD_QUOTED=jt;it.foldFlowLines=md});var st=_(Rs=>{"use strict";var ee=M(),pe=Is(),Yt=(n,e)=>({indentAtStart:e?n.indent.length:n.indentAtStart,lineWidth:n.options.lineWidth,minContentWidth:n.options.minContentWidth}),zt=n=>/^(%|---|\.\.\.)/m.test(n);function hd(n,e,t){if(!e||e<0)return!1;let i=e-t,r=n.length;if(r<=i)return!1;for(let s=0,o=0;s<r;++s)if(n[s]===`
...
+(?!
|$)`,"g")}catch{ti=/\n+(?!\n|$)/g}function Xt({comment:n,type:e,value:t},i,r,s){let{blockQuote:o,commentString:a,lineWidth:c}=i.options;if(!o||/\n[\t ]+$/.test(t))return Me(t,i);let l=i.indent||(i.forceBlockIndent||zt(t)?"  ":""),u=o==="literal"?!0:o==="folded"||e===ee.Scalar.BLOCK_FOLDED?!1:e===ee.Scalar.BLOCK_LITERAL?!0:!hd(t,c,l.length);if(!t)return u?`|
`:`>
...
`&&(m=m.slice(0,-1)),m=m.replace(ti,`$&${l}`));let p=!1,y,T=-1;for(y=0;y<t.length;++y){let k=t[y];if(k===" ")p=!0;else if(k===`
...
$&`).replace(/(?:^|\n)([\t ].*)(?:([\n\t ]*)\n(?![\n\t ]))?/g,"$1$2").replace(/\n+/g,`$&${l}`),v=!1,N=Yt(i,!0);o!=="folded"&&e!==ee.Scalar.BLOCK_FOLDED&&(N.onOverflow=()=>{v=!0});let b=pe.foldFlowLines(`${E}${k}${m}`,l,pe.FOLD_BLOCK,N);if(!v)return`>${w}
${l}${b}`}return t=t.replace(/\n+/g,`$&${l}`),`|${w}
${l}${E}${t}${m}`}function gd(n,e,t,i){let{type:r,value:s}=n,{actualString:o,implicitKey:a,indent:c,indentStep:l,inFlow:u}=e;if(a&&s.includes(`
...
`).replace(/^[\s\S]*?\/\*[*!]/,"").replace(/\*\/[\s\S]*$/,""),f={text:il(d),endLine:l},m=rl(f.text),g=sl(f.text);if(g.length>0){let S;for(let w of g)switch(w.kind){case"define":{let k={id:w.id,title:w.title,header:e},v=m.ingroup??s[s.length-1];v&&(k.parent=v),r.push(k),S=w.id;break}case"add":S=w.id;break;case"open":s.push(S??s[s.length-1]??""),S=void 0;break;case"close":s.pop();break}if(!m.brief&&m.params.length===0&&m.retvals.length===0){o=l;continue}}let p=ll(t,l+1);if(!p){o=l;continue}let y=cl(p.text);if(!y){o=l;continue}let T=m.ingroup??s.filter(Boolean)[s.filter(Boolean).length-1],E={name:y.name,kind:y.kind,signature:y.signature,params:m.params,returns:m.returns,retvals:m.retvals,header:e,line:p.line+1,deprecated:m.deprecated};m.brief&&(E.brief=m.brief),m.detail&&(E.detail=m.detail),T&&(E.group=T),m.since&&(E.since=m.since),i.push(E),o=l}return{symbols:i,groups:r}}import{existsSync as xt,readFileSync as dl,realpathSync as fl}from"node:fs";import{delimiter as ul,join as Je,resolve as pl}from"node:path";import{spawnSync as Qr}from"node:child_process";function Wr(n,e){if(n.includes("/")||n.includes("\\"))return xt(n)?pl(n):void 0;for(let t of(e??"").split(ul).filter(Boolean)){let i=Je(t,n);if(xt(i))return i}}function ml(n){let e=Wr("west",n.PATH);if(e)try{let i=(dl(fl(e),"utf8").split(/\r?\n/,1)[0]??"").match(/^#!\s*(\S+)(?:\s+(.+))?$/);return i?i[1]?.endsWith("/env")&&i[2]?Wr(i[2].trim().split(/\s+/,1)[0],n.PATH):i[1]&&xt(i[1])?i[1]:void 0:void 0}catch{return}}function Zr(n){return[n.PYTHON_EXECUTABLE,ml(n),"python3","python"].filter((e,t,i)=>!!e&&i.indexOf(e)===t)}function es(n=process.env){for(let e of Zr(n))if(Qr(e,["-c","import sys; assert sys.version_info >= (3, 10)"],{encoding:"utf8",env:{...n,PYTHONDONTWRITEBYTECODE:"1"}}).status===0)return e;throw new Error("This index adapter requires Python 3.10 or newer. Set PYTHON_EXECUTABLE to a supported interpreter and retry.")}function xe(n,e=process.env){let t=Je(n,"scripts","kconfig"),i=Je(n,"scripts","dts","python-devicetree","src");if([Je(t,"kconfiglib.py"),Je(i,"devicetree","edtlib.py")].filter(a=>!xt(a)).length>0)throw new Error("The selected Zephyr tree is missing its semantic ingestion libraries (scripts/kconfig/kconfiglib.py and/or scripts/dts/python-devicetree). Use a complete Zephyr checkout and retry.");let s=Zr(e),o=["import sys",`sys.path.insert(0, ${JSON.stringify(t)})`,`sys.path.insert(0, ${JSON.stringify(i)})`,"import kconfiglib","import yaml","from devicetree import edtlib","assert sys.version_info >= (3, 10)"].join("; ");for(let a of s)if(Qr(a,["-c",o],{encoding:"utf8",env:{...e,PYTHONDONTWRITEBYTECODE:"1"}}).status===0)return a;throw new Error("Semantic index creation requires Python 3.10 or newer with PyYAML, plus the Kconfiglib and devicetree libraries shipped by the selected Zephyr tree. Activate the project's west virtual environment or set PYTHON_EXECUTABLE to its Python interpreter, then retry.")}import{existsSync as hl,readdirSync as gl}from"node:fs";import{join as yl,relative as bl,sep as ts}from"node:path";var El=new Set([".git","node_modules","__pycache__",".venv","build","twister-out"]);function*Z(n,e={}){if(!hl(n))return;let t=e.skipDirs??El,i=e.skipPrefixes??[],r=[n];for(;r.length>0;){let s=r.pop(),o;try{o=gl(s,{withFileTypes:!0})}catch(a){throw new Error(`Failed to read source directory ${s}: ${a instanceof Error?a.message:String(a)}`)}for(let a of o){let c=yl(s,a.name),l=Ce(bl(n,c));if(a.isDirectory()){if(t.has(a.name)||i.some(u=>l===u||l.startsWith(`${u}/`)))continue;r.push(c)}else if(a.isFile()){if(i.some(u=>l.startsWith(`${u}/`))||e.match&&!e.match(a.name))continue;yield l}else if(a.isSymbolicLink())throw new Error(`Refusing symbolic link in indexed source tree: ${c}`)}}}function Ce(n){return ts==="/"?n:n.split(ts).join("/")}function Al(n,e){if(!Tl(De(e,"index.xml")))throw new Error(`The Doxygen XML directory has no index.xml: ${e}`);let t=_l(De(vl(),"zephyr-ai-api-")),i=De(t,"api-export.py");try{wl(i,Jr,{mode:384});let r=kl(es(),[i,"--xml",e],{encoding:"utf8",maxBuffer:512*1024*1024,env:{...process.env,PYTHONDONTWRITEBYTECODE:"1"}});if(r.status!==0)throw new Error(`Doxygen XML export failed.
${r.stderr.trim().split(`
`).slice(-12).join(`
`)}`);let s=JSON.parse(r.stdout);return s.symbols=s.symbols.map(o=>{let a=o.header.replaceAll("\\","/"),c="/include/zephyr/",l=a.lastIndexOf(c);return{...o,header:l>=0?`include/zephyr/${a.slice(l+c.length)}`:a}}),s}finally{Sl(t,{recursive:!0,force:!0})}}function ns(n,e){if(e)return Al(n,e);let t=De(n,"include","zephyr"),i=[],r=[],s=[];for(let a of Z(t,{skipPrefixes:["internal","arch/arm/internal"],match:c=>c.endsWith(".h")})){let c;try{c=Nl(De(t,a),"utf8")}catch(d){throw new Error(`Cannot read public API header ${De(t,a)}: ${d instanceof Error?d.message:String(d)}`)}let l=`include/zephyr/${a}`,u=Hr(c,l);for(let d of u.symbols){if(d.kind==="function"&&d.signature.includes("=")){s.push({path:`${l}:${d.line}`,reason:"fallback-initializer-artifact"});continue}let f=d.signature.indexOf("["),m=d.signature.indexOf("(");if(d.kind==="function"&&f>=0&&(m<0||f<m)){s.push({path:`${l}:${d.line}`,reason:"fallback-array-declarator-artifact"});continue}if(d.kind==="macro"&&/^#define\s+[A-Z][A-Z0-9_]*_H_*$/.test(d.signature)){s.push({path:`${l}:${d.line}`,reason:"fallback-include-guard"});continue}i.push(d)}r.push(...u.groups)}i.sort((a,c)=>a.name.localeCompare(c.name));let o=new Map;for(let a of r)(!o.has(a.id)||a.title&&!o.get(a.id).title)&&o.set(a.id,a);return{symbols:i,groups:[...o.values()],mode:"header-fallback",report:{discovered:i.length+o.size+s.length+1,indexed:i.length+o.size,intentionallyExcluded:[...s,{path:"include/zephyr/internal",reason:"private-header-policy"}],warnings:[{code:"header-fallback",message:"Doxygen XML was not supplied; API results are an incomplete header-comment catalogue."}],errors:[]}}}import{existsSync as Ol,mkdtempSync as Il,rmSync as Rl,writeFileSync as xl}from"node:fs";import{tmpdir as Cl}from"node:os";import{dirname as rs,join as Bn}from"node:path";import{spawnSync as Dl}from"node:child_process";var is=`#!/usr/bin/env python3
...
`).split(`
`),c=[];for(let l=0;l<a.length;l++){let u=a[l],d=u.match(/^(\s*)\.\.\s+(include|literalinclude|only)::\s*(.*)$/);if(!d){c.push(u);continue}let f=d[1].length,m=d[2],g=d[3].trim(),p=[],y=l+1;for(;y<a.length;y++){let N=a[y];if(N.trim()===""){p.push(N);continue}if(N.match(/^\s*/)[0].length<=f)break;p.push(N)}if(l=y-1,m==="only"){if(/\bhtml\b/.test(g)){let N=p.map(O=>O.trim()?O.slice(Math.min(O.length,f+3)):""),b=Or(n,s,N.join(`
`),i,r);c.push(...b.split(`
`).map(O=>`${" ".repeat(f)}${O}`))}continue}let T=Om(p),E=wm(Nm(s),g);if(!Tm(E))throw new Error(`include target not found: ${g}`);if(_m(E).isSymbolicLink())throw new Error(`include target is a symbolic link: ${g}`);let S=Ar(n),w=Ar(E),k=Lr(S,w);if(k===".."||k.startsWith(`..${nc}`))throw new Error(`include escapes the Zephyr tree: ${g}`);let v=Im(sc(w,"utf8"),T);if(i.push({path:Lr(S,w).replaceAll(nc,"/"),startLine:v.start,endLine:v.end,directive:m}),m==="literalinclude"){let N=T.language??Sm(E).slice(1);c.push(`${" ".repeat(f)}.. code-block:: ${N}`,"",...v.text.split(`
`).map(b=>`${" ".repeat(f+3)}${b}`))}else{let N=Or(S,w,v.text,i,o);c.push(...N.split(`
`).map(b=>`${" ".repeat(f)}${b}`))}}return c.join(`
`)}function rc(n,e,t,i){let r=[],s=tc(n,e);for(let o of Z(s,{skipDirs:vm,match:a=>a.endsWith(".rst")})){let a=`${e}/${o}`,c=tc(s,o);i.discovered++;try{let l=sc(c,"utf8"),u=[{path:a,startLine:1,endLine:l.split(/\r?\n/).length,directive:"page"}],d=Or(n,c,l,u),f=ec(d),m=f.chunks.filter(g=>g.body.trim()!=="").map((g,p)=>({...g,ord:p}));if(m.length===0){let g=Lm(d);if(g.length>0){let p=f.title||ic(a);m=[{heading:p,headingPath:[p],ord:0,body:`Contained documentation pages:
${g.map(y=>`- ${y}`).join(`
`)}`}]}}if(m.length===0){i.intentionallyExcluded.push({path:a,reason:"no-retrievable-content"});continue}r.push({path:a,url:km(a,t),title:f.title||ic(a),area:Am(a),labels:f.labels,chunks:m,origins:u}),i.indexed++}catch(l){i.errors.push({path:a,code:"rst-preprocess",message:l.message})}}return r}function oc(n,e){let t={discovered:0,indexed:0,intentionallyExcluded:[],warnings:[],errors:[]},i=[...rc(n,"doc",e,t),...rc(n,"boards",e,t)];if(t.errors.length>0){let r=t.errors.slice(0,12).map(s=>`${s.path}: ${s.message}`).join(`
`);throw new Error(`Documentation preprocessing failed for ${t.errors.length} source(s).
${r}`)}return{pages:i,report:t}}import{existsSync as xm,mkdtempSync as Cm,rmSync as Dm,writeFileSync as Pm}from"node:fs";import{tmpdir as qm}from"node:os";import{join as Cn}from"node:path";import{spawnSync as $m}from"node:child_process";var ac=`#!/usr/bin/env python3
...
if __name__ == "__main__":
    main()
`;var cc=new Map;function lc(n,e=[]){let t=JSON.stringify([n,[...e].sort()]),i=cc.get(t);if(i)return i;let r=Cn(n,"scripts","kconfig","kconfiglib.py");if(!xm(r))throw new Error("The selected Zephyr tree does not provide scripts/kconfig/kconfiglib.py.");let s=Cm(Cn(qm(),"zephyr-ai-kconfig-")),o=Cn(s,"kconfig-export.py"),a=Cn(s,"generated");try{Pm(o,ac,{mode:384});let c=[o,"--zephyr",n,"--build-dir",a];for(let f of e)c.push("--module",f);let l=$m(xe(n),c,{cwd:n,encoding:"utf8",maxBuffer:256*1024*1024,env:{...process.env,PYTHONDONTWRITEBYTECODE:"1"}});if(l.status!==0){let f=l.stderr.trim().split(`
`).slice(-8).join(`
`);throw new Error(`Zephyr Kconfiglib export failed.
${f}`)}let u=JSON.parse(l.stdout),d={symbols:u.symbols,choices:u.choices,filesScanned:u.files.length,warnings:u.warnings};return cc.set(t,d),d}finally{Dm(s,{recursive:!0,force:!0})}}var uc=Br(Nr(),1);import{existsSync as Pn,readFileSync as fc,statSync as Mm}from"node:fs";import{dirname as dc,join as Le}from"node:path";var Um=64*1024,Fm=160*1024;function pc(n){return/^(prj.*\.conf|sysbuild\.conf|CMakeLists\.txt|Kconfig|sample\.yaml|README\.rst)$/.test(n)?!0:/\.(overlay|conf|dts|dtsi|c|h|cpp|hpp|yml|yaml)$/.test(n)&&/^(boards|snippets|src)\//.test(n)}function Bm(n,e){let t=[],i=[],r=Fm;for(let s of e){if(!pc(s))continue;let o=Le(n,s);try{if(Mm(o).size>Um){i.push({path:s,reason:"file-size-limit"});continue}let a=fc(o,"utf8");if(Buffer.byteLength(a)>r){i.push({path:s,reason:"sample-size-budget"});continue}r-=Buffer.byteLength(a),t.push({path:s,text:a})}catch(a){throw new Error(`Failed to capture sample file ${o}: ${a.message}`)}}return{contents:t,exclusions:i}}function Km(n){return Array.isArray(n)?n:typeof n=="string"?[n]:[]}function Dn(n){return Km(n).filter(e=>typeof e=="string")}function jm(n){let e=[],t=i=>{Pn(Le(n,i))&&e.push(i)};for(let i of["sample.yaml","prj.conf","CMakeLists.txt","Kconfig","sysbuild.conf","README.rst"])t(i);for(let i of["src","boards","snippets"]){let r=Le(n,i);if(Pn(r))try{e.push(...[...Z(r,{match:s=>pc(`${i}/${s}`)})].map(s=>`${i}/${s}`))}catch{}}return e}function mc(n){let e=[],t=new Set;for(let i of["samples","snippets"]){let r=Le(n,i);if(Pn(r))for(let s of Z(r,{match:o=>o==="sample.yaml"})){let o=Le(r,s),a=null;try{let N=(0,uc.parse)(fc(o,"utf8"),{logLevel:"silent"});if(!N||typeof N!="object"||Array.isArray(N))throw new Error("expected a YAML mapping");a=N}catch(N){throw new Error(`Failed to parse sample metadata ${s}: ${N.message}`)}let c=dc(o),l=Ce(Le(i,dc(s)));if(t.has(l))continue;t.add(l);let u=a.sample&&typeof a.sample=="object"?a.sample:{},d=a.tests&&typeof a.tests=="object"?a.tests:{},f=a.common&&typeof a.common=="object"&&!Array.isArray(a.common)?a.common:{},m=new Set,g=new Set,p=new Set,y=new Set,T=N=>{for(let b of Dn(N.tags))m.add(b);if(typeof N.tags=="string")for(let b of N.tags.split(/\s+/).filter(Boolean))m.add(b);for(let b of Dn(N.depends_on))g.add(b);for(let b of Dn(N.integration_platforms))p.add(b);for(let b of Dn(N.platform_allow))y.add(b)};T(f);for(let N of Object.values(d))!N||typeof N!="object"||T({...f,...N});let E=jm(c),{contents:S,exclusions:w}=Bm(c,E),k=S.map(N=>N.path),v={path:l,name:typeof u.name=="string"?u.name:l.split("/").pop(),tags:[...m].sort(),dependsOn:[...g].sort(),integrationPlatforms:[...p].sort(),platformAllow:[...y].sort(),files:k,contents:S,exclusions:w};typeof u.description=="string"&&(v.description=u.description),Pn(Le(c,"README.rst"))&&(v.docPath=`${l}/README.rst`),e.push(v)}}return e.sort((i,r)=>i.path.localeCompare(r.path)),e}import{createHash as Rr}from"node:crypto";import{existsSync as $n,readFileSync as qn,realpathSync as At,statSync as Qm}from"node:fs";import{basename as gc,dirname as Zm,join as Oe,relative as eh,resolve as th}from"node:path";import{spawnSync as bc}from"node:child_process";import{createHash as Xm}from"node:crypto";import{existsSync as Ym,lstatSync as zm,readFileSync as Vm,readlinkSync as Gm,realpathSync as Jm}from"node:fs";import{join as Hm}from"node:path";import{spawnSync as Wm}from"node:child_process";function Ir(n,e){let t=Wm("git",["-C",n,...e],{encoding:"utf8",maxBuffer:268435456,stdio:["ignore","pipe","ignore"]});return t.status===0?t.stdout.trim():null}function hc(n){let e=Jm(n),t=Ir(e,["rev-parse","HEAD"]);if(!t)return null;let i=Ir(e,["diff","--binary","HEAD"])??"",r=(Ir(e,["ls-files","--others","--exclude-standard"])??"").split(`
`).filter(s=>!!s&&s!==".zephyr-ai-managed.json").sort().map(s=>{let o=Hm(e,s);if(!Ym(o))return{path:s,missing:!0};try{let a=zm(o);return a.isSymbolicLink()?{path:s,symlink:Gm(o)}:a.isFile()?{path:s,sha256:Xm("sha256").update(Vm(o)).digest("hex")}:{path:s,special:a.mode}}catch{return{path:s,unreadable:!0}}});return{commit:t,dirty:!!(i||r.length),stateFingerprint:_e({commit:t,diff:i,untracked:r})}}function nh(n,e){let t=bc("git",["-C",n,...e],{encoding:"utf8",stdio:["ignore","pipe","ignore"]});return t.status===0?t.stdout.trim():null}function ih(n){let e=qn(Oe(n,"VERSION"),"utf8"),t=s=>e.match(new RegExp(`^${s}\\s*=\\s*(.*)$`,"m"))?.[1]?.trim()??"",i=[t("VERSION_MAJOR"),t("VERSION_MINOR"),t("PATCHLEVEL")].join("."),r=t("EXTRAVERSION");return r?`${i}-${r}`:i}function rh(n){let e=th(n);for(;;){if($n(Oe(e,".west","config")))return e;let t=Zm(e);if(t===e)return;e=t}}function sh(n){if(!n)return;let e=bc("west",["manifest","--freeze"],{cwd:n,encoding:"utf8",stdio:["ignore","pipe","ignore"]});if(e.status===0&&e.stdout.trim())return Rr("sha256").update(e.stdout).digest("hex");let t="",i="west.yml";try{let o=qn(Oe(n,".west","config"),"utf8");t=o.match(/^\s*path\s*=\s*(.+)$/m)?.[1]?.trim()??"",i=o.match(/^\s*file\s*=\s*(.+)$/m)?.[1]?.trim()??i}catch{}let s=[...t?[Oe(n,t,i)]:[],Oe(n,"west.yml"),Oe(n,"west.yaml")].find($n);return s?Rr("sha256").update(qn(s)).digest("hex"):void 0}function yc(n){let e=At(n),t=hc(e);if(t)return{name:gc(e),...t};let i=["VERSION","west.yml","zephyr/module.yml","module.yml"].map(r=>Oe(e,r)).filter($n).map(r=>{let s=Qm(r);return{path:eh(e,r),bytes:s.size,sha256:Rr("sha256").update(qn(r)).digest("hex")}});return{name:gc(e),markers:i}}function Ec(n){let e=At(n.zephyrRoot),t=n.projectRoot&&$n(n.projectRoot)?At(n.projectRoot):void 0,i=nh(e,["rev-parse","HEAD"]);if(!i)throw new Error(`Cannot determine the Git commit for the Zephyr tree at ${e}.`);let r=rh(t??e),s=sh(r),o=n.modules.map(f=>yc(f)),a=_e(o),c=yc(e),l=String(c.stateFingerprint??_e(c)),u=n.pinnedCommit===i&&c.dirty===!1?"pinned-upstream":r?"west-workspace":"explicit-tree",d={descriptorVersion:Kr,schemaVersion:Rt,builderVersion:jr,sourceKind:u,...t?{projectRoot:t}:{},zephyrRoot:e,zephyrVersion:ih(e),zephyrCommit:i,zephyrTreeFingerprint:l,...s?{westManifestHash:s}:{},moduleFingerprint:a,...n.boardTarget?{boardTarget:n.boardTarget}:{},...n.applicationRoot?{applicationRoot:At(n.applicationRoot)}:{},...n.buildDirectory?{buildDirectory:At(n.buildDirectory)}:{},coverage:{docs:{complete:n.modules.length===0,note:n.modules.length?"Module documentation is not indexed.":void 0},kconfig:{complete:!1,note:"Catalogue index; generated and application-local symbols require resolved context."},bindings:{complete:n.modules.length===0&&!t&&!n.applicationRoot,note:n.modules.length||t||n.applicationRoot?"Application-local or undisclosed module binding roots may not be indexed.":void 0},boards:{complete:n.modules.length===0,note:n.modules.length?"Module board roots are not indexed.":void 0},samples:{complete:n.modules.length===0,note:n.modules.length?"Module samples are not indexed.":void 0},api:{complete:!!n.apiSemantic&&n.modules.length===0,note:n.apiSemantic?n.modules.length?"Module public headers are not indexed.":void 0:"Doxygen XML was not supplied; the API catalogue is an incomplete header fallback."},resolvedBuild:{complete:!1,note:n.buildDirectory?"Build identity is recorded, but resolved .config and final devicetree values are not ingested.":"No resolved build output was supplied or ingested."}}};return{...d,createdAt:new Date().toISOString(),contextFingerprint:Yr(d)}}function mh(n){let e=ie(process.cwd()),t={zephyr:process.env.ZEPHYR_BASE??z(e,".cache","zephyr"),modules:[],quiet:!1,requireDoxygen:!1,requirePinned:!1,projectRoot:process.env.CLAUDE_PROJECT_DIR??process.env.ZEPHYR_AI_PROJECT_ROOT,pluginData:process.env.CLAUDE_PLUGIN_DATA??process.env.ZEPHYR_AI_PLUGIN_DATA};for(let i=0;i<n.length;i++){let r=n[i];switch(r){case"--zephyr":t.zephyr=ie(n[++i]);break;case"--out":t.out=ie(n[++i]);break;case"--project-root":t.projectRoot=ie(n[++i]);break;case"--plugin-data":t.pluginData=ie(n[++i]);break;case"--board":t.boardTarget=n[++i];break;case"--application":t.applicationRoot=ie(n[++i]);break;case"--build-dir":t.buildDirectory=ie(n[++i]);break;case"--api-xml":t.apiXml=ie(n[++i]);break;case"--require-doxygen":t.requireDoxygen=!0;break;case"--require-pinned":t.requirePinned=!0;break;case"--modules":t.modules.push(ie(n[++i]));break;case"--quiet":case"-q":t.quiet=!0;break;case"--help":case"-h":console.log("Usage: zephyr-ai-ingest [--zephyr <path>] [--project-root <path>] [--out <path>] [--modules <path>]... [--board <target>] [--application <path>] [--build-dir <path>] [--api-xml <dir>] [--require-doxygen] [--require-pinned] [--quiet]"),process.exit(0);break;default:throw new Error(`Unknown argument: ${r}`)}}return t.zephyr=ie(t.zephyr),t}function hh(){for(let n of[z(process.cwd(),"zephyr.lock.json"),z(process.cwd(),"..","..","zephyr.lock.json")])try{return JSON.parse(dh(n,"utf8"))}catch{}return{}}function Nc(n){return n===void 0?null:JSON.stringify(n)}function Cr(n){let e=lh(n,"r");try{ah(e)}finally{oh(e)}}function Sc(n){try{Cr(n)}catch{}}function gh(n,e){let t=fh(n,{withFileTypes:!0}).filter(r=>r.isDirectory()&&/^[a-f0-9]{64}$/.test(r.name)).flatMap(r=>{let s=z(n,r.name),o=z(s,"zephyr.db");if(!Mn(o))return[];let a=z(s,"last-used");return[{fingerprint:r.name,directory:s,usedAt:wc(Mn(a)?a:o).mtimeMs}]}).sort((r,s)=>s.usedAt-r.usedAt),i=new Set([e,...t.filter(r=>r.fingerprint!==e).slice(0,4).map(r=>r.fingerprint)]);for(let r of t)i.has(r.fingerprint)||xr(r.directory,{recursive:!0,force:!0})}function yh(){let n=mh(process.argv.slice(2)),e=X=>{n.quiet||process.stderr.write(`${X}
`)};if(!Mn(z(n.zephyr,"VERSION")))throw new Error(`${n.zephyr} does not look like a Zephyr tree (no VERSION file).
Run 'npm run fetch:zephyr' first, or pass --zephyr <path>.`);xe(n.zephyr);let t=hh();if(n.requireDoxygen&&!n.apiXml)throw new Error("Release API ingestion requires Doxygen XML. Run npm run build:api-xml, then pass --api-xml .cache/doxygen/xml.");let i=Ec({zephyrRoot:n.zephyr,...n.projectRoot?{projectRoot:n.projectRoot}:{},modules:n.modules,...t.commit?{pinnedCommit:t.commit}:{},...n.boardTarget?{boardTarget:n.boardTarget}:{},...n.applicationRoot?{applicationRoot:n.applicationRoot}:{},...n.buildDirectory?{buildDirectory:n.buildDirectory}:{},apiSemantic:!!n.apiXml}),r=i.zephyrVersion;if(n.requirePinned&&(!t.commit||i.sourceKind!=="pinned-upstream"))throw new Error(`The requested pinned index build requires commit ${t.commit??"<missing lock>"}, but the selected tree is ${i.zephyrCommit}. The checkout must also have no tracked or untracked source changes. Run npm run fetch:zephyr -- --force or omit --require-pinned for an explicit workspace index.`);let s=`https://docs.zephyrproject.org/${r}/`,o,a=n.out;if(!a&&n.pluginData)if(i.projectRoot){let X=z(n.pluginData,"indexes","projects",Xr(i.projectRoot));a=z(X,i.contextFingerprint,"zephyr.db"),o=z(X,"active.json")}else a=z(n.pluginData,"indexes","defaults",i.zephyrCommit,String(i.schemaVersion),"zephyr.db");a??=z(ie(process.cwd()),"index","zephyr.db"),e(`Indexing Zephyr ${r} from ${n.zephyr}`);let c=Date.now(),l=Date.now(),{pages:u,report:d}=oc(n.zephyr,s),f=u.reduce((X,ae)=>X+ae.chunks.length,0);e(`  docs      ${u.length} pages, ${f} sections (${Date.now()-l} ms)`);let m=Date.now(),g=lc(n.zephyr,n.modules);e(`  kconfig   ${g.symbols.length} symbols from ${g.filesScanned} files (${Date.now()-m} ms)`);let p=Date.now(),y=[z(n.zephyr,"dts","bindings"),...n.modules.map(X=>z(X,"dts","bindings")).filter(Mn)],{bindings:T,fragments:E,report:S}=os(y),w=X=>X.properties.length+X.children.reduce((ae,Un)=>ae+w(Un),0),k=T.reduce((X,ae)=>X+w(ae),0);e(`  bindings  ${T.length} compatibles, ${k} properties, ${E} fragments (${Date.now()-p} ms)`);let v=Date.now(),N=Wa(n.zephyr),b=Qa(n.zephyr),O=N.reduce((X,ae)=>X+ae.targets.length,0);e(`  boards    ${N.length} boards, ${O} targets, ${b.length} SoCs (${Date.now()-v} ms)`);let J=Date.now(),B=mc(n.zephyr);e(`  samples   ${B.length} (${Date.now()-J} ms)`);let q=Date.now(),L=ns(n.zephyr,n.apiXml);e(`  api       ${L.symbols.length} symbols, ${L.groups.length} groups, ${L.mode} (${Date.now()-q} ms)`),ch(Lt(a),{recursive:!0});let K=z(Lt(a),`.${_c()}.zephyr.db.tmp`),A,Dr=!1;try{A=new ph(K),A.exec(Vr);let X=Date.now();A.exec("BEGIN");let ae=A.prepare("INSERT INTO doc (path, url, title, area, labels) VALUES (?, ?, ?, ?, ?)"),Un=A.prepare(`INSERT INTO doc_chunk (doc_id, anchor, heading, heading_path, ord, title, body)
     VALUES (?, ?, ?, ?, ?, ?, ?)`),vc=A.prepare("INSERT INTO doc_origin (doc_id, path, start_line, end_line, directive) VALUES (?, ?, ?, ?, ?)");for(let h of u){let x=ae.run(h.path,h.url,h.title,h.area,JSON.stringify(h.labels)),I=Number(x.lastInsertRowid);for(let R of h.origins)vc.run(I,R.path,R.startLine,R.endLine,R.directive);for(let R of h.chunks)Un.run(I,R.anchor??null,R.heading,R.headingPath.join(" > "),R.ord,h.title,R.body)}let kc=A.prepare(`INSERT INTO kconfig
       (name, type, prompt, help, defaults, depends, selects, implies, ranges,
        defined_in, menu_path, is_choice, choice, n_defs, has_prompt)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`),Fn=A.prepare("INSERT INTO kconfig_edge (from_sym, to_sym, kind) VALUES (?, ?, ?)"),Ot=new Map;for(let h of g.symbols){let x=h.definitions.flatMap(D=>D.defaults.map($=>({value:$.value.display,...$.condition.display!=="y"?{cond:$.condition.display}:{}}))),I=h.definitions.map(D=>D.condition.display).filter((D,$,Gc)=>D!=="y"&&Gc.indexOf(D)===$),R=h.definitions.flatMap(D=>D.selects.map($=>({value:$.target,...$.condition.display!=="y"?{cond:$.condition.display}:{}}))),j=h.definitions.flatMap(D=>D.implies.map($=>({value:$.target,...$.condition.display!=="y"?{cond:$.condition.display}:{}}))),ze=h.definitions.flatMap(D=>D.ranges.map($=>({low:$.low.display,high:$.high.display,...$.condition.display!=="y"?{cond:$.condition.display}:{}}))),H=h.definitions.find(D=>D.prompt)?.prompt??"",Ie=h.definitions.find(D=>D.menuPath.length>0)?.menuPath.join(" > ")??"",U=kc.run(h.name,h.type??null,H,h.help??"",JSON.stringify(x),JSON.stringify(I),JSON.stringify(R),JSON.stringify(j),JSON.stringify(ze),JSON.stringify(h.definitions.map(D=>({file:D.file,line:D.line}))),Ie,h.choice?1:0,h.choice??null,h.definitions.length,h.hasPrompt?1:0);Ot.set(h.name,Number(U.lastInsertRowid));for(let D of R)Fn.run(h.name,D.value,"select");for(let D of j)Fn.run(h.name,D.value,"imply");let Re=D=>[...D.kind==="symbol"&&D.value?[D.value]:[],...(D.children??[]).flatMap(Re)];for(let D of h.definitions)for(let $ of Re(D.condition))Fn.run(h.name,$,"depends")}let Ac=A.prepare("INSERT INTO kconfig_expr (kind, value, display, left_id, right_id) VALUES (?, ?, ?, ?, ?)"),Pr=new Map,re=h=>{if(!h)return null;let x=Q(h),I=Pr.get(x);if(I!==void 0)return I;let R=h.children??[],j=Number(Ac.run(h.kind,h.value??null,h.display,re(R[0]??null),re(R[1]??null)).lastInsertRowid);return Pr.set(x,j),j},Lc=A.prepare(`INSERT INTO kconfig_definition
     ...
     VALUES (?, ?, ?, ?, ?)`);for(let h of g.symbols){let x=Ot.get(h.name);for(let I of h.definitions){let R=Number(Lc.run(x,I.file,I.line,I.prompt,JSON.stringify(I.menuPath),re(I.condition),re(I.promptCondition),I.isMenuconfig?1:0,I.isConfigDefault?1:0).lastInsertRowid);for(let j of I.defaults)Oc.run(R,re(j.value),re(j.condition),j.order);for(let[j,ze]of[["select",I.selects],["imply",I.implies]])for(let H of ze)Ic.run(R,j,H.target,Ot.get(H.target)??null,re(H.condition),H.order);for(let j of I.ranges)Rc.run(R,re(j.low),re(j.high),re(j.condition),j.order)}}let xc=A.prepare("INSERT INTO kconfig_choice (stable_id, name, type, definitions) VALUES (?, ?, ?, ?)"),Cc=A.prepare("INSERT INTO kconfig_choice_member (choice_id, symbol_id) VALUES (?, ?)");for(let h of g.choices){let x=Number(xc.run(h.id,h.name,h.type,JSON.stringify(h.definitions)).lastInsertRowid);for(let I of new Set(h.members)){let R=Ot.get(I);R!==void 0&&Cc.run(x,R)}}let Dc=A.prepare(`INSERT INTO dt_binding
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`),qc=A.prepare("INSERT INTO text_pool (text) VALUES (?)"),qr=new Map,$c=h=>{if(!h)return null;let x=qr.get(h);if(x!==void 0)return x;let I=Number(qc.run(h).lastInsertRowid);return qr.set(h,I),I};for(let h of T){let x=h.compatible,I=(H,Ie=0,U="")=>[...H.properties.map(Re=>({level:Ie,childPath:U,property:Re})),...H.children.flatMap((Re,D)=>I(Re,Ie+1,U?`${U}/${D}`:String(D)))],R=I(h),j=Dc.run(x,h.path,h.description??"",h.bus===void 0?null:typeof h.bus=="string"?h.bus:JSON.stringify(h.bus),h.onBus??null,JSON.stringify(h.cells),JSON.stringify(h.includes),R.map(({property:H})=>H.name).join(" "),R.length,x.includes(",")?x.split(",")[0]:null),ze=Number(j.lastInsertRowid);for(let{level:H,childPath:Ie,property:U}of R)Pc.run(ze,H,U.name,U.type??null,U.required?1:0,$c(U.description),Nc(U.default),U.enum===void 0?null:JSON.stringify(U.enum),Nc(U.const),U.deprecated?1:0,U.specifierSpace??null,U.inheritedFrom??null,JSON.stringify(U.provenance??{}),JSON.stringify(U.constraints??{}),Ie)}let Mc=A.prepare(`INSERT INTO board
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);for(let h of N){let x=h.socs.map(I=>I.name);Mc.run(h.name,h.fullName??"",h.vendor??"",h.dir,h.arch??null,h.ram??null,h.flash??null,JSON.stringify(h.socs),x.join(" "),JSON.stringify(h.targets),h.targets.map(I=>I.identifier).join(" "),JSON.stringify(h.revisions),h.defaultRevision??null,JSON.stringify(h.supported),h.supported.join(" "),h.docPath??null)}let Uc=A.prepare("INSERT INTO soc (name, series, family, vendor, dir, cpuclusters) VALUES (?, ?, ?, ?, ?, ?)");for(let h of b)Uc.run(h.name,h.series??null,h.family??null,h.vendor??null,h.dir,JSON.stringify(h.cpuclusters));let Fc=A.prepare(`INSERT INTO sample
       (path, name, description, tags, tags_text, depends_on, integration_platforms,
        platform_allow, files, doc_path)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`),Bc=A.prepare("INSERT INTO sample_file (sample_id, path, text) VALUES (?, ?, ?)"),$r=A.prepare("INSERT INTO sample_platform (sample_id, platform, evidence) VALUES (?, ?, ?)");for(let h of B){let x=Fc.run(h.path,h.name,h.description??"",JSON.stringify(h.tags),h.tags.join(" "),JSON.stringify(h.dependsOn),JSON.stringify(h.integrationPlatforms),JSON.stringify(h.platformAllow),JSON.stringify(h.files),h.docPath??null),I=Number(x.lastInsertRowid);for(let R of h.contents)Bc.run(I,R.path,R.text);for(let R of h.integrationPlatforms)$r.run(I,R,"integration");for(let R of h.platformAllow)$r.run(I,R,"allowlist")}let Kc=A.prepare(`INSERT INTO api_symbol
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);for(let h of L.symbols)Kc.run(h.name,h.kind,h.signature,h.brief??"",h.detail??"",JSON.stringify(h.params),JSON.stringify(h.returns),JSON.stringify(h.retvals),h.group??null,h.since??null,h.deprecated?1:0,h.header,h.line,h.doxygenId??null,h.compoundId??null,h.docAnchor??null);let jc=A.prepare("INSERT INTO api_group (gid, title, parent, header) VALUES (?, ?, ?, ?)");for(let h of L.groups)jc.run(h.id,h.title,h.parent??null,h.header);let Xc=A.prepare("INSERT INTO meta (key, value) VALUES (?, ?)"),Yc={schema_version:String(zr),zephyr_version:r,zephyr_commit:i.zephyrCommit,zephyr_tag:i.sourceKind==="pinned-upstream"?t.tag??"":"",source_path:n.zephyr,source_kind:i.sourceKind,index_descriptor:Q(i),context_fingerprint:i.contextFingerprint,module_fingerprint:i.moduleFingerprint,doc_base_url:s,built_at:new Date().toISOString(),ingest_version:"0.1.0",count_docs:String(u.length),count_doc_chunks:String(f),report_docs:Q(d),count_kconfig:String(g.symbols.length),report_kconfig:Q({discovered:g.symbols.length+g.choices.length,indexed:g.symbols.length+g.choices.length,intentionallyExcluded:[],warnings:[{code:"source-files",message:`Kconfiglib evaluated ${g.filesScanned} source files.`},...g.warnings.map(h=>({code:"kconfiglib",message:h}))],errors:[]}),count_bindings:String(T.length),count_dt_properties:String(k),report_bindings:Q(S),count_boards:String(N.length),count_board_targets:String(O),count_socs:String(b.length),report_boards:Q({discovered:N.length+O+b.length,indexed:N.length+O+b.length,intentionallyExcluded:[],warnings:[{code:"report-units",message:"Counts include board, target, and SoC records."}],errors:[]}),count_samples:String(B.length),report_samples:Q({discovered:B.length+B.reduce((h,x)=>h+x.contents.length+x.exclusions.length,0),indexed:B.length+B.reduce((h,x)=>h+x.contents.length,0),intentionallyExcluded:B.flatMap(h=>h.exclusions.map(x=>({path:`${h.path}/${x.path}`,reason:x.reason}))),warnings:[{code:"report-units",message:"Counts include sample records and eligible attached files."}],errors:[]}),count_api:String(L.symbols.length),api_ingest_mode:L.mode,report_api:Q(L.report)};for(let[h,x]of Object.entries(Yc))Xc.run(h,x);A.exec("COMMIT"),e(`  written   (${Date.now()-X} ms)`);let zc=Date.now();A.exec(Gr),e(`  indexed   full-text (${Date.now()-zc} ms)`),A.exec("VACUUM"),A.exec("PRAGMA optimize");let Mr=String(A.prepare("PRAGMA integrity_check").get()?.integrity_check??""),Ur=A.prepare("PRAGMA foreign_key_check").all();if(Mr!=="ok"||Ur.length>0)throw new Error(`Index verification failed (integrity=${Mr}, foreign-key violations=${Ur.length}).`);for(let[h,x]of[["doc_fts","doc_chunk"],["kconfig_fts","kconfig"],["dt_fts","dt_binding"],["board_fts","board"],["sample_fts","sample"],["api_fts","api_symbol"]]){let I=Number(A.prepare(`SELECT COUNT(*) AS n FROM ${h}`).get()?.n),R=Number(A.prepare(`SELECT COUNT(*) AS n FROM ${x}`).get()?.n);if(I!==R)throw new Error(`Index verification failed: ${h} has ${I} rows; ${x} has ${R}.`)}if(A.close(),A=void 0,Cr(K),Tc(K,a),Sc(Lt(a)),Dr=!0,o){let h=`${o}.${_c()}.tmp`;uh(h,`${Q({contextFingerprint:i.contextFingerprint,relativePath:`${i.contextFingerprint}/zephyr.db`,activatedAt:new Date().toISOString()})}
`,{flag:"wx"}),Cr(h),Tc(h,o),Sc(Lt(o)),gh(Lt(o),i.contextFingerprint)}let Vc=wc(a).size;e(`Done in ${((Date.now()-c)/1e3).toFixed(1)} s -> ${a} (${(Vc/1024/1024).toFixed(1)} MiB)`)}finally{try{A?.close()}catch{}Dr||(xr(K,{force:!0}),xr(`${K}-journal`,{force:!0}))}}try{yh()}catch(n){process.stderr.write(`zephyr-ai-ingest: ${n.message}
`),process.exit(1)}
