...
+(?!
|$)`,"g")}catch{Sr=/\n+(?!\n|$)/g}function an({comment:n,type:e,value:t},r,i,s){let{blockQuote:o,commentString:a,lineWidth:c}=r.options;if(!o||/\n[\t ]+$/.test(t))return Ze(t,r);let l=r.indent||(r.forceBlockIndent||ln(t)?"  ":""),p=o==="literal"?!0:o==="folded"||e===ce.Scalar.BLOCK_FOLDED?!1:e===ce.Scalar.BLOCK_LITERAL?!0:!pu(t,c,l.length);if(!t)return p?`|
`:`>
...
$&`).replace(/(?:^|\n)([\t ].*)(?:([\n\t ]*)\n(?![\n\t ]))?/g,"$1$2").replace(/\n+/g,`$&${l}`),k=!1,A=cn(r,!0);o!=="folded"&&e!==ce.Scalar.BLOCK_FOLDED&&(A.onOverflow=()=>{k=!0});let N=Se.foldFlowLines(`${b}${v}${m}`,l,Se.FOLD_BLOCK,A);if(!k)return`>${T}
