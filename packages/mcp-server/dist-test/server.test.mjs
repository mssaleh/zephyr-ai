...
// test/server.test.ts
var REPO = resolve(process.cwd(), "..", "..");
var SERVER = join(REPO, "plugin", "mcp", "zephyr-mcp.mjs");
var INDEX = process.env["ZEPHYR_AI_INDEX"] ?? join(REPO, "index", "zephyr.db");
var ready = existsSync(SERVER) && existsSync(INDEX);
...
var Client = class {
  ...
  request(method, params = {}) {
    ...
    return new Promise((resolvePromise, reject) => {
      const timer = setTimeout(() => reject(new Error(`timeout waiting for ${method}`)), 2e4);
...
describe("MCP server", { skip: !ready && "run `npm run build` and build the index first" }, () => {
  ...
  describe("error handling", () => {
    it("reports a missing required argument as a tool error, not a protocol error", async () => {
      ...
      strictEqual(res.error, void 0, "must not be a JSON-RPC error");
  ...
  describe("sample tools", () => {
    it("returns the prj.conf contents stored in the index", async () => {
      ...
      const included = res.structured["filesIncluded"];
  ...
  describe("index_status", () => {
    it("reports the indexed version and coverage", async () => {
      ...
      ok(Number(res.structured["counts"]["Boards"]) > 900);
    });
  });
});
