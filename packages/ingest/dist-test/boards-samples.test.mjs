...
// test/boards-samples.test.ts
import { ok, strictEqual } from "node:assert/strict";
import { existsSync as existsSync4 } from "node:fs";
import { join as join4 } from "node:path";
import { describe, it } from "node:test";

// src/sources/boards.ts
var import_yaml = __toESM(require_dist(), 1);
import { existsSync as existsSync2, readFileSync, readdirSync as readdirSync2 } from "node:fs";
...
function safeYaml(path) {
  try {
    const parsed = (0, import_yaml.parse)(readFileSync(path, "utf8"), { logLevel: "silent" });
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      throw new Error("expected a YAML mapping");
    }
    return parsed;
  } catch (error) {
    throw new Error(`Failed to parse board/SoC metadata ${path}: ${error.message}`);
  }
}
function asArray(v) {
...
function officialBoards(root) {
  const script = join2(root, "scripts", "list_boards.py");
  if (!existsSync2(script)) throw new Error("The selected Zephyr tree has no scripts/list_boards.py.");
  let exported;
  for (const python of [process.env["PYTHON_EXECUTABLE"], "python3", "python"]) {
    ...
    if (!exported.error || exported.error.code !== "ENOENT") break;
  }
  if (!exported || exported.status !== 0) {
    throw new Error(
      `Board ingestion requires Python 3 plus the PyYAML and jsonschema modules used by Zephyr scripts/list_boards.py. The official board exporter failed: ${exported?.stderr.trim() ?? "Python was not found."}`
    );
  }
  const result = /* @__PURE__ */ new Map();
  for (const line of exported.stdout.split("\n").filter(Boolean)) {
    const fields = line.split("@@").filter(Boolean).map((field) => field.split(";"));
    const value = (name2) => fields.find(([label]) => label === name2)?.slice(1) ?? [];
...
function readTargets(boardDir) {
  const targets = [];
  let entries;
  try {
    entries = readdirSync2(boardDir);
  } catch {
    return targets;
  }
  for (const entry of entries) {
    ...
    const doc = safeYaml(join2(boardDir, entry));
    const common = {
    ...
    const variants = doc["variants"] && typeof doc["variants"] === "object" && !Array.isArray(doc["variants"]) ? doc["variants"] : {};
    for (const [identifier, override] of Object.entries(variants)) {
      const details = override && typeof override === "object" && !Array.isArray(override) ? override : {};
      targets.push({
        identifier,
        ...common,
        toolchains: asStrings(details["toolchain"]).length ? asStrings(details["toolchain"]) : common.toolchains,
        supported: [.../* @__PURE__ */ new Set([...common.supported, ...asStrings(details["supported"])])]
      });
    }
  }
...
function collectBoards(root) {
  ...
  for (const rel of walk(join2(root, "boards"), {
    match: (name) => name === "board.yml" || name === "board.yaml"
  })) {
    const abs = join2(root, "boards", rel);
    ...
    const relDir = toPosix(join2("boards", dirname(rel)));
    const targets = readTargets(boardDir);
    const documentation = [...walk(join2(boardDir, "doc"), { match: (name) => name.endsWith(".rst") })];
    const preferredDoc = documentation.includes("index.rst") ? "index.rst" : documentation.sort()[0];
    const docPath = preferredDoc ? `${relDir}/doc/${preferredDoc}` : void 0;
    for (const entry of entries) {
      ...
      const socs = asArray(entry["socs"]).flatMap((s) => {
        ...
        return [
          {
            name: rec["name"],
            variants: asArray(rec["variants"]).flatMap(
              (v) => v && typeof v === "object" && typeof v["name"] === "string" ? [v["name"]] : []
            ),
            cpuclusters: asArray(rec["cpuclusters"]).flatMap(
              (c) => c && typeof c === "object" && typeof c["name"] === "string" ? [c["name"]] : []
            )
          }
        ];
      });
      ...
      const generated = identifiers.map((identifier) => ({
        identifier,
        toolchains: [],
        supported: []
      }));
      ...
      const effective = [...targetByName.values()].sort(
      ...
      const ram = effective.find((t) => t.ram !== void 0)?.ram;
      if (ram !== void 0) record.ram = ram;
      const flash = effective.find((t) => t.flash !== void 0)?.flash;
      if (flash !== void 0) record.flash = flash;
      boards.push(record);
    }
  }
...
var MAX_SAMPLE_BYTES = 160 * 1024;
function shouldStore(rel) {
  if (/^(prj.*\.conf|sysbuild\.conf|CMakeLists\.txt|Kconfig|sample\.yaml|README\.rst)$/.test(rel)) return true;
  return /\.(overlay|conf|dts|dtsi|c|h|cpp|hpp|yml|yaml)$/.test(rel) && /^(boards|snippets|src)\//.test(rel);
}
function readContents(sampleDir, files) {
  const out = [];
  const exclusions = [];
  let budget = MAX_SAMPLE_BYTES;
  for (const rel of files) {
    if (!shouldStore(rel)) continue;
    const abs = join3(sampleDir, rel);
    try {
      ...
      throw new Error(`Failed to capture sample file ${abs}: ${error.message}`);
    }
  }
...
function asStrings2(v) {
  return asArray2(v).filter((x) => typeof x === "string");
}
function interestingFiles(sampleDir) {
  const out = [];
  const push = (rel) => {
    if (existsSync3(join3(sampleDir, rel))) out.push(rel);
  };
  for (const f of ["sample.yaml", "prj.conf", "CMakeLists.txt", "Kconfig", "sysbuild.conf", "README.rst"]) {
    push(f);
  }
  for (const dir of ["src", "boards", "snippets"]) {
    const abs = join3(sampleDir, dir);
...
function collectSamples(root) {
  ...
  for (const subdir of ["samples", "snippets"]) {
    const base = join3(root, subdir);
    if (!existsSync3(base)) continue;
    for (const rel of walk(base, { match: (name) => name === "sample.yaml" })) {
      const abs = join3(base, rel);
      let doc = null;
      try {
        const parsed = (0, import_yaml2.parse)(readFileSync2(abs, "utf8"), { logLevel: "silent" });
        if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
          throw new Error("expected a YAML mapping");
        }
        doc = parsed;
      } catch (error) {
        throw new Error(`Failed to parse sample metadata ${rel}: ${error.message}`);
      }
      const sampleDir = dirname2(abs);
      const relDir = toPosix(join3(subdir, dirname2(rel)));
      if (seen.has(relDir)) continue;
      seen.add(relDir);
      const meta = doc["sample"] && typeof doc["sample"] === "object" ? doc["sample"] : {};
      const tests = doc["tests"] && typeof doc["tests"] === "object" ? doc["tests"] : {};
      const common = doc["common"] && typeof doc["common"] === "object" && !Array.isArray(doc["common"]) ? doc["common"] : {};
      const tags = /* @__PURE__ */ new Set();
      const dependsOn = /* @__PURE__ */ new Set();
      const integration = /* @__PURE__ */ new Set();
      const platformAllow = /* @__PURE__ */ new Set();
      const addMetadata = (t) => {
        for (const x of asStrings2(t["tags"])) tags.add(x);
        if (typeof t["tags"] === "string") {
          for (const x of t["tags"].split(/\s+/).filter(Boolean)) tags.add(x);
        }
        for (const x of asStrings2(t["depends_on"])) dependsOn.add(x);
        for (const x of asStrings2(t["integration_platforms"])) integration.add(x);
        for (const x of asStrings2(t["platform_allow"])) platformAllow.add(x);
      };
      addMetadata(common);
      for (const value of Object.values(tests)) {
        if (!value || typeof value !== "object") continue;
        addMetadata({ ...common, ...value });
      }
      ...
      if (existsSync3(join3(sampleDir, "README.rst"))) record.docPath = `${relDir}/README.rst`;
      samples.push(record);
    }
  }
...
var haveTree = existsSync4(join4(ZEPHYR, "boards"));
