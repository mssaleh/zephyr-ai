...
function parseIncludes(raw) {
  ...
  for (const item of raw) {
    ...
    out.push({
      ...
      childAllowlist: childRec ? asStringArray(childRec["property-allowlist"]) : void 0,
      childBlocklist: childRec ? asStringArray(childRec["property-blocklist"]) : void 0
    });
  }
  return out;
}
function applyFilters(props, allowlist, blocklist) {
  let out = props;
  if (allowlist) {
...
function mergeProperties(inherited, local) {
  ...
  for (const p of local) {
    ...
    const merged = { ...existing, ...stripUndefined(p) };
...
function stampOrigin(props, path) {
...
function resolveBinding(path, loader, stack = []) {
  if (stack.includes(path)) return null;
  ...
  for (const spec of includes) {
    ...
    if (inc.bus !== void 0) inheritedBus = inc.bus;
    if (inc.onBus !== void 0) inheritedOnBus = inc.onBus;
    if (inc.children.length > 0) {
      const filteredChildren = inc.children.map((child, idx) => ({
        ...child,
        properties: stampOrigin(
          idx === 0 ? applyFilters(child.properties, spec.childAllowlist, spec.childBlocklist) : child.properties,
          inc.path
        )
      }));
  ...
  const compatible = typeof raw["compatible"] === "string" ? raw["compatible"] : void 0;
  ...
  if (bus !== void 0 && (typeof bus === "string" || Array.isArray(bus))) {
    result.bus = bus;
  }
...
function resolveChildBinding(raw, parentPath, loader, stack) {
  const includes = parseIncludes(raw["include"]);
  let inheritedProps = [];
  let inheritedChildren = [];
  const includePaths = [];
  ...
  const children = nested ? mergeChildren(inheritedChildren, [resolveChildBinding(nested, parentPath, loader, stack)]) : inheritedChildren;
  const out = {
...
function mergeChildren(a, b) {
  ...
  for (let i = 0; i < depth; i++) {
    ...
    out.push({
      ...
      includes: [.../* @__PURE__ */ new Set([...left.includes, ...right.includes])],
      children: []
    });
  }
  return out;
...
function safeParseYaml(text) {
  try {
    const parsed = (0, import_yaml.parse)(text, { logLevel: "silent" });
...
function mockLoader(files) {
  return {
    resolve: (name) => name in files ? name : void 0,
    load: (path) => files[path] !== void 0 ? safeParseYaml(files[path]) : null
  };
}
describe("parseIncludes", () => {
  ...
  it("accepts filter objects", () => {
    ...
    strictEqual(specs[1].allowlist, void 0);
  });
});
describe("resolveBinding", () => {
  it("flattens a multi-level include chain", () => {
    const loader = mockLoader({
      ...
      "spi-controller.yaml": "include: base.yaml\nbus: spi\nproperties:\n  clock-frequency:\n    type: int\n    description: Clock rate\n",
      "st,stm32-spi-common.yaml": "include: spi-controller.yaml\nproperties:\n  cs-gpios:\n    type: phandle-array\n",
      "st,stm32-spi.yaml": 'description: STM32 SPI controller\ncompatible: "st,stm32-spi"\ninclude: st,stm32-spi-common.yaml\n'
    });
...
describe("against the real Zephyr tree", { skip: !haveTree && "Zephyr tree not fetched" }, () => {
  ...
  it("indexes the expected number of bindings", () => {
    ...
    ok(bindings.length > 3e3, `expected >3000 compatibles, got ${bindings.length}`);
